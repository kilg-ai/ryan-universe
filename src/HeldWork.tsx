...
export function HeldWork({inquiry,records,available,onSave,onReview}:{inquiry:Inquiry;records:RecordItem[];available:boolean;onSave:HeldSave;onReview:(i:Inquiry,r:RecordItem,p:HeldPacket)=>Promise<boolean>}) {
 ...
 {!available&&<p role="status" aria-live="polite">Shared workspace unavailable. Reconnect before saving review work.</p>}
 <label className="field">Review type<select value={kind} disabled={busy} onChange={e=>{setKind(e.target.value as HeldKind);setBody('');setContactChecked(false);}}><option value="contract">Contract draft / brief</option><option value="follow-up">Follow-up draft</option></select></label>
 ...
 <form aria-busy={busy} onSubmit={e=>void save(e)}>
 <label className="field">Review owner<input required maxLength={100} disabled={busy} value={owner} onChange={e=>setOwner(e.target.value)}/></label>
 <label className="field">Review date<input required type="date" disabled={busy} value={due} onChange={e=>setDue(e.target.value)}/></label>
 ...
 {kind==='follow-up'&&<><label><input type="checkbox" checked={suppressed} disabled={busy} onChange={e=>setSuppressed(e.target.checked)}/> Complaint or contact restriction — stop drafting</label><label><input type="checkbox" checked={contactChecked} disabled={busy} onChange={e=>setContactChecked(e.target.checked)}/> I checked the existing conversation and contact restrictions for this draft</label></>}
 <label className="field">{kind==='contract'?'Proposed contract text / brief':'Exact follow-up text'}<textarea required rows={7} maxLength={1000} disabled={busy} value={body} onChange={e=>setBody(e.target.value)}/></label>
 <button className="button primary" disabled={!available||busy||(kind==='follow-up'&&(suppressed||!contactChecked))}>{busy?'Saving…':`Save version ${(latest?.packet.version||0)+1} — held for review`}</button>
 </form></details>;
}
